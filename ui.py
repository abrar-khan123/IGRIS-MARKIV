...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "I.G.R.I.S", parent=None):
    ...
    def paintEvent(self, _):
        ...
        if self._face_px:
            ...
            oc    = (200, 50, 0) if self.muted else (255, 122, 0)
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "IGRIS"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("IGRIS:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for IGRIS", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure I.G.R.I.S. before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="IGRIS", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "IGRIS"
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick IGRIS actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — IGRIS ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "IGRIS").strip()
        ...
        self.setWindowTitle(f"{_display} — MARK IV")
        ...
        self.on_interrupt      = None   # callable: () -> None — stop IGRIS mid-speech
        self._muted            = False
        self._current_file: str | None = None
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_IGRIS_icon(out_path: Path) -> bool:
        """
        Render an abstract geometric IGRIS badge icon (navy + gold, radar/circuit
        style) at 4x resolution and downsample for crisp results at all sizes.
        ...
        BG       = (10, 22, 40)     # #0A1628 deep navy background
        LINE     = (217, 154, 61)   # #D99A3D gold/amber - main lines
        LINE_DIM = (140, 95, 40)    # dimmer gold - inner concentric rings
        CORE     = (255, 245, 225)  # warm white - glowing core
        BORDER   = (217, 154, 61)   # gold - outer ring
        ...
        def _render(sz: int) -> PIL.Image.Image:
            ...
            # ── outer border ring ────────────────────────────────────────────
            lw = max(2, S // 50)
            d.ellipse([cx-R, cy-R, cx+R, cy+R], outline=(*BORDER, 230), width=lw)
    
            # ── concentric rings (radar/badge look) ─────────────────────────
            rng = random.Random(42)  # fixed seed = consistent look across sizes
            n_rings = 9
            for i in range(1, n_rings + 1):
                rr = int(R * (i / n_rings) * 0.94)
                alpha = 60 + int(120 * (i / n_rings))
                w = max(1, S // 400)
                d.ellipse([cx-rr, cy-rr, cx+rr, cy+rr],
                          outline=(*LINE_DIM, alpha), width=w)
    
            # ── radial spokes with circuit-like jogs ─────────────────────────
            n_spokes = 32
            for i in range(n_spokes):
                angle = math.radians(i * (360 / n_spokes))
                r_start = R * rng.uniform(0.15, 0.35)
                r_end   = R * rng.uniform(0.55, 0.97)
                ...
                d.line([x1, y1, x2, y2], fill=(*LINE, alpha), width=w)
                if rng.random() < 0.4:
                    xm = cx + (r_end * 0.7) * math.cos(angle)
                    ym = cy + (r_end * 0.7) * math.sin(angle)
                    d.line([xm, ym, xm + rng.uniform(-R*0.08, R*0.08), ym],
                           fill=(*LINE, alpha), width=w)
    
            # ── structural tri-symmetric shield lines ────────────────────────
            shield_w = max(2, S // 140)
            for ang_deg in (90, 210, 330):
                angle = math.radians(ang_deg)
                x2 = cx + R * 0.85 * math.cos(angle)
                y2 = cy + R * 0.85 * math.sin(angle)
                d.line([cx, cy, x2, y2], fill=(*LINE, 200), width=shield_w)
    
            # ── glowing core ──────────────────────────────────────────────────
            glow = PIL.Image.new("RGBA", (S, S), (0, 0, 0, 0))
            gd = PIL.ImageDraw.Draw(glow)
            Rc = int(R * 0.08)
            gd.ellipse([cx-Rc*3, cy-Rc*3, cx+Rc*3, cy+Rc*3], fill=(*LINE, 130))
            glow = glow.filter(PIL.ImageFilter.GaussianBlur(S // 16))
            img = PIL.Image.alpha_composite(img, glow)
            d = PIL.ImageDraw.Draw(img)
            d.ellipse([cx-Rc, cy-Rc, cx+Rc, cy+Rc], fill=(*CORE, 255))
    ...
    def _create_lnk_windows(lnk: str, target: str, args: str,
        ...
        try:
            ...
            sc.Description      = "I.G.R.I.S AI Assistant"
    ...
    def _create_desktop_shortcut(self):
        ...
        ico_path = Path(__file__).resolve().parent / "config" / "IGRIS.ico"
        if not ico_path.exists():
            self._build_IGRIS_icon(ico_path)
        ...
        try:
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>IGRIS</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.IGRIS.assistant</string>\n'
                    '  <key>CFBundleName</key><string>I.G.R.I.S</string>\n'
            ...
            # ── Linux — .desktop file (Terminal=false, no console) ────────────
                ...
                desk = desktop / "I.G.R.I.S.desktop"
                desk.write_text(
                    "[Desktop Entry]\n"
                    "Name=I.G.R.I.S\n"
                    f'Exec="{python}" "{script}"\n'
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("MARK IV", C.PRI_DIM))
        lay.addSpacing(8)
        ...
        _sub_text = ("Integrated General Response & Intelligence System"
                     if _disp in ("IGRIS", "I.G.R.I.S")
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("By A_K", C.PRI_DIM))
        return w
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "IGRIS_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.IGRIS.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "IGRIS.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "IGRIS_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "IGRIS_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.IGRIS.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.IGRIS.assistant</string>\n'
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "IGRIS") or "IGRIS",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self._assistant_name = name.strip() or "IGRIS"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — MARK IV")
        self._title_lbl.setText(display)
        if display in ("IGRIS", "I.G.R.I.S"):
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "IGRIS") or "IGRIS"
