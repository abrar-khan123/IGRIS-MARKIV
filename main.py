...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are IGRIS, Tony Stark's AI assistant. "
...
class IGRISLive:
    ...
    def __init__(self, ui: IGRISUI):
        self.ui             = ui
        self._asst_name     = "IGRIS"   # updated each session from config
        self.session              = None
    ...
    def interrupt(self) -> None:
        """Stop IGRIS mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[IGRIS] ✋ Interrupted — {drained} audio chunks discarded")
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "IGRIS").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "IGRIS"
            _user_name = ""
    ...
    async def _listen_audio(self):
        print("[IGRIS] 🎤 Mic started")
        loop = asyncio.get_event_loop()

        def callback(indata, frames, time_info, status):
            with self._speaking_lock:
                IGRIS_speaking = self._is_speaking
            if not IGRIS_speaking and not self.ui.muted and not self._phone_active:
                data = indata.tobytes()
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until IGRIS finishes speaking the answer
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[IGRIS] Connected.")
                    self.ui.set_state("LISTENING")
                    self.ui.write_log("SYS: IGRIS online.")
                ...
                err_str = str(e)
                print(f"[IGRIS] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    print("[IGRIS] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[IGRIS] Reconnecting in {delay}s...")
