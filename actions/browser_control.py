...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".IGRIS_profiles" / browser
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".IGRIS_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using IGRIS profile")
                IGRIS = str(Path.home() / ".IGRIS_profiles" / "firefox_IGRIS")
                Path(IGRIS).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(IGRIS, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".IGRIS_profiles" / "safari")
        ...
        # IGRIS otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        IGRIS_profile = str(Path.home() / ".IGRIS_profiles" / self.browser_name)
        Path(IGRIS_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with IGRIS profile: {IGRIS_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(IGRIS_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with IGRIS profile "
